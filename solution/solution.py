...
def compare_models(prompt: str) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get("gpt-4o", {"output": 0.010})
...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        total_tokens += count_tokens(user_msg, model=OPENAI_MODEL) + count_tokens(
            reply, model=OPENAI_MODEL
        )
        total_cost += estimate_cost(user_msg, reply, model=OPENAI_MODEL)[
            "total_cost"
        ]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        lines.append(
            f"{p:<40} | {g:<40} | {m:<40} | {r.get('gpt4o_latency', 0.0):<15.2f} | {r.get('mini_latency', 0.0):<15.2f}"
        )
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Entry point — demo chạy thật (cần OPENAI_API_KEY hoặc Ollama)
# ---------------------------------------------------------------------------
